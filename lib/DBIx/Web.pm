...
''	=>{''		=>['',		'']
	...
	,'owner'	=>['Owner',	'Owner of record, user name']
	,'orole'	=>['Owners',	'Role of owner of the record or additional owner']
	,'owners'	=>['Owners',	'Owners of record, users and groups, comma delimited']
	...
	,'comment'	=>['Comment',	"Comment text or HTML; 'urlh://', 'urlr://', 'urlf://' URL protocols may be used to denote relative URLs to host, this script, file store; starting with <where>condition</where> may be used for embedded query"]
...
,'ru'	=>{''		=>['',		'']
	...
	,'comment'	=>['Коммент',	"Текст или HTML комментария; гиперссылки могут быть начаты с 'urlh://' (компьютер), 'urlr://' (этот сценарий), 'urlf://' (файловое хранилище); начало <where>условие</where> может использоваться для встроенной выборки"]
...
sub ugfile {	# Users/groups caching, 'AuthGroupFile' file write/refresh
 ...
 local(*FG, *FL, *FW);
 open(FG, "+>>$fg.tmp")
	|| ($s && &{$s->{-die}}($s->lng(0, 'ugfile') .": open('$fg.tmp') -> $!"))
	|| croak("open('<$fg.tmp') -> $!");
 open(FL, "+>>$fl.tmp")
	|| ($s && &{$s->{-die}}($s->lng(0, 'ugfile') .": open('$fl.tmp') -> $!"))
	|| croak("open('<$fl.tmp') -> $!");
...
sub ldap {	# LDAP connection
 ...
 return	(&{$s->{-die}}('LDAP connection undefined'))
	if !$a;
 my $r;
 if(ref($a) eq 'CODE') {
 }
...
sub ldapSearch {# LDAP search
 ...
 return	(&{$_[0]->{-die}}("ldapSearch(" .join(',', map{"$_=>" .$a1{$_}} keys %a1) .') ->' .$r->error))
	if $r->code;
 $r
}
...
sub ldapEntry {	# LDAP search and return entry
 ...
 my $r =$_[0]->ldapSearch($#_ <2 
	? ('filter'=>	$_[1] !~/[=]/
			...
			: $_[1])
	: @_[1..$#_]);
 return	(&{$_[0]->{-die}}('ldapRead('. join(', ',@_[1..$#_]) .'-> sevaral entries found'))
	if $r->count >1;
 $r->entry(0);
}
...
sub mdeRoles {	# Table user roles list
 ...
 return(qw(all author authors actor actors principal principals user users)) 
	if !$_[1];
...
sub recNew {    # Create new record to be inserted into database
 ...
 my $p =$a->{-proto} || ((grep {$_} values %{$a->{-key}}) ? $s->recRead(%$a, -data=>undef, -test=>1) : {});
...
sub cgiRun {	# Execute CGI query
 ...
 while(1) {	# Determine / Delegate operation object requested / Execute
	...
	return(&{$s->{-die}}($s->lng(0,'cgiRun') .": Operation object '$on' not found") && undef) 
			if !$om;
...
sub cgiParse {	# Parse CGI call parameters
 ...
 foreach my $k (keys %$d) {
	...
	elsif ($k =~/^_(cmd|cmg|frmCall|frmName\d*|frmLso|frmLsc|recNew|recRead|recPrint|recXML|recEdit|recIns|recUpd|recDel|recForm|recList|recQBF|submit.*|app.*|form|key|proto|urm|qkey|qwhere|qurole|quname|qftext|qversion|qorder|qkeyord|qlist|qlimit|qftwhere|qftord|qftlimit|edit|backc|login|print|refresh|xml|style|frame)(?:\.[xXyY]){0,1}$/i) {
...
sub urlOpt {	# Option URL
 ...
 $s->urlCat($s->cgi->url
	,(map {	my $n =$_;
		...
		&& ($s->{-pcmd}->{$_} ne '')
		&& ($n =$_ =~/^-/ ? '_' .substr($_,1) : $_)
		&& ($n !~/_(?:frmName|cmg|cmh|backc|ui)/i)
		&& !exists($v{$n})
		? ($n =>  (ref($s->{-pcmd}->{$_}) 
			? $s->strdata($s->{-pcmd}->{$_}) 
			: $s->{-pcmd}->{$_}))
	...
	, $s->{-c}->{-htmlstart}  =
		...
		: $s->cgi->start_html(
			...
			,-style	=> ".Form {margin-top:0px; font-size: 8pt; font-family: Verdana, Helvetica, Arial, sans-serif; }\n"
				...
				#.".ListTableFocus {border-style: solid; border-bottom-width: 1px; border-top-width: 0px; border-left-width: 0px; border-right-width: 0px; background-color: red; }\n"
...
sub htmlMenu {	# Screen menu bar
 ...
 if	($g eq 'recList') {		# View menu items
	...
	|| (grep {(ref($_) eq 'ARRAY') && ($_->[0] =~/^\Q$on\E\+/)
 ...
 : ("\n<div class=\"$cs MenuDiv\"><table class=\"$cs\" cellpadding=0><tr>\n"
	...
	. $mi .'</nobr></td></tr>'
	."\n" 
	."</table>\n<table class=\"$cs\" cellpadding=0 width=100%>"
...
sub cgiForm {	# Print CGI screen form
 ...
 $s->output('<div id="_FormArea"><table>'
 ...
 if ($qm) {	# Query condition fields
	my $q =$m->{-query} ||{};
	$s->output('<input type="hidden" name="_frmLso" value="' 
		.$s->htmlEscape(ref($c->{-frmLso}) ? $s->strdata($c->{-frmLso}) : $c->{-frmLso})
		.'" />')
		if 0 && $c->{-frmLso} && ($c->{-frmLso} !~/^-/);
	$s->output('<input type="hidden" name="_frmLsc" value="' 
		.$s->htmlEscape(ref($c->{-frmLsc}) ? $s->strdata($c->{-frmLsc}) : $c->{-frmLsc})
		.'" />')
		if 0;
	$s->output("<table>\n") if $lt;
	$lt =0; $lr =1;
 ...
 $s->output(!$lt ? "</table></div>\n" : "</div>\n")
	if !$c->{-xml};
 $s->output("<script for=\"window\" event=\"onload\">{if(self.name=='BOTTOM'){var e; for(e in document.getElementById('_FormArea').getElementsByTagName('a')){if(!e.target||(e.target=='_self')){e.target='TOP'; e.normalize}}}}</script>\n")
	if 0 && !$c->{-xml}; # !!! future frameset implementations
 $s
}
...
sub urlCGBtr {	# Translate -cgibus special urls
		# (string) -> translated
		...
		# 		-> _form=...
		# id=		-> _key=...
 return($_[1])	if !$_[0]->{-cgibus} 
		||($_[1] !~/^(?:host|urlh|url|urlr|fsurl|urlf)/);
 my $v =$_[1];
 if ($v =~/^(?:url|urlr)/) {
	$v =~s/_tcb_cmd=-sel/'_cmd=recRead&_form=' .$_[0]->{-pcmd}->{-form}/ge;
	$v =~s/_tcb_cmd=-lst/'_cmd=recList&_form=' .$_[0]->{-pcmd}->{-table}/ge;
	$v =~s/_tsw_FTEXT=/_qftext=/;
	$v =~s/_tsw_WHERE=/_qwhere=/;
	$v =~s/&id=/&_key=/g;
 }
 $v
}
...
sub cgiDDLB {	# CGI Drop-down list box
 ...
 if	($s->{-pdta}->{$ne}  ||$s->{-pcmd}->{$ne} ||$s->{-pdta}->{$nc}) {
	...
	."window.document.DBIx_Web." .$nf .'.focus();}</script>');
 }
 ...
 my $fs =sub{
	...
	.(!$_[0]
	? "k=window.document.DBIx_Web.$nf.value +String.fromCharCode(window.event.keyCode);"
	: $_[0] eq '1'
	? "window.document.DBIx_Web.$nf.focus(); k=window.document.DBIx_Web.$nf.value =String.fromCharCode(window.event.keyCode); "
	: $_[0] eq '2'
 ...
 && defined($rf) && !$rf && $s->{-pdta}->{$ne}) {
	$s->output("<script for=\"window\" event=\"onload\">{"
	."window.document.DBIx_Web.${nl}.style.display=\"none\";"
	."}</script>");
	return($s)
 }
 $s->output("<br />\n");
 ...
 $s->output($s->cgi->submit(-name=>$nc
		...
		, -style=>"width: 2em;"),"\n");
 $s->output('<script for="window" event="onload">'
	.(!$ml ? &$fs(4) : "{window.document.DBIx_Web.$nl.focus()}")
	."</script>\n");
}
...
sub cgiQDflt {	# Default query arguments fulfill
 ...
 $c =$s->{-pcmd} if !$c;
 unless ($c->{-qkey} ||defined($c->{-qwhere}) ||defined($c->{-qurole})) {
...
sub cgiQInherit { # Inherit cgi query attributes if needed
 ...
 $tm =	  !$q->{-table}
	...
	if !$tm;
 # return(&{$s->{-die}}("cgiQInherit -> no table meta")) if !$tm;
 $q->{-meta} =
	   (ref($q->{-meta}) && $q->{-meta}) 
	...
	if !$qm;
 my $qmv =$qm ||$q->{-meta};
 # return(&{$s->{-die}}("cgiQInherit -> no query meta")) if !$qmv;
 if ($qm) {
 ...
 foreach my $m ($q, $qmv, ($qmv ne $tm) ? $tm : ()) {
	next	if !$m;
	# $_[0]->logRec('***cgiQInherit',$m->{-lbl} || 'query');
	if (!$q->{-data}) {
	$q->{-field}=$m->{-field}
		if !$q->{-field};
	$q->{-data} =
...
sub htmlMQH {	# Menu Query Hyperlink
 ...
 my $qw=	# writing query joining required
	...
	, !defined($a->{-xpar}) || ($a->{-xpar} eq '1')
	? (map {$s->{-pcmd}->{$_}
		? ($_ => $s->{-pcmd}->{$_})
		: ()	} qw (-qftext -frmLsc))
	: !$a->{-xpar} || ($a->{-xpar} !~/^\d/)
	? (map {($_ =~/^-(?:q|frmLsc)/) && $s->{-pcmd}->{$_}
		? ($_ => $s->{-pcmd}->{$_})
		: () 	} keys %{$s->{-pcmd}})
	: ()};
 if($a->{-xpar} && ($a->{-xpar} !~/^\d/)) {
	foreach my $k (ref($a->{-xpar}) ? @{$a->{-xpar}} : $a->{-xpar}) {
		next if exists($qq->{$k});
		delete $qw->{$k};
	}
 }
 if ($a->{-xkey}) {
	foreach my $k (ref($a->{-xkey}) ? @{$a->{-xkey}} : $a->{-xkey}) {
		next if $qq->{-qkey} && exists($qq->{-qkey}->{$k});
		delete $qw->{-qkey}->{$k} if $qw->{-qkey};
	}
 }
 my $ql=200;	# query length limit
 if (length($s->urlCmd('', %$qw)) >$ql) {
	delete $qw->{-qkey};
	delete $qw->{-qwhere} if length($s->urlCmd('', %$qw)) >$ql;
 }
 my $qm=1;	# query match
...
sub cgiList {	# List queried records
 ...
 $b =	# bondaries:
	...
	: !$b
	? ["\n<table $hstl >\n"
# onfocus=\"this.className =this.className+&quot; &quot;+&quot;ListTableFocus&quot;\" onblur=\"this.className=this.className.replace(&quot;ListTableFocus&quot;, &quot&quot;)\"
		,"<tr>\n", "<td align=\"left\" valign=\"top\" $hstl >", "<a $hstl href=\"", $c->{-frame} ? '" target="' .$c->{-frame} .'">' : '">', '</a>', '', "</td>\n", "</tr>\n", "</table>\n"]
	: $b =~/<select/ # !"</select>\n"

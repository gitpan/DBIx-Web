...
sub urlCat {
 my $r =$_[1] .'?';
 for (my $i =2; $i <$#_; $i+=2) {$r .=urlEscape($_[0], $_[$i]) .'=' .urlEscape($_[0], $_[$i+1]) .'&'}
 chop($r);
 $r
}
...
#!!! commented below failures CGI.pm !!!
#$ENV{'TMPDIR'}=$_[0]->pthForm('tmp');
#$TempFile::TMPDIRECTORY =$_[0]->pthForm('tmp');
#$TempFile::TMPDIRECTORY =~s/\//\\/g if $^O eq 'MSWin32';
 $_[0]->{-cgi} =eval("use CGI; CGI->new()")
	|| &{$_[0]->{-die}}($_[0]->lng(0,'cgi') .": CGI::new() -> $@");
}
...
sub w32IISdpsn {# deimpersonate Microsoft IIS impersonated process
		# !!! Problems may be. Implement '-fswtr' login also?
		# 'Win32::API' used.
...
sub dbmSeek {	# Select records from dbm file using -key and -where
 my ($s, $a, $e) =@_;
 my $m =$s->{-table}->{$a->{-table}};
 my $i =$m->{-key};
 my $k =($a->{-key} ? [map {$a->{-key}->{$_}} grep {exists $a->{-key}->{$_}} @$i] : []);
 my $wk={ $a->{-key} 
	? (map {($_=>$a->{-key}->{$_})} (grep {my $v =$_; !grep {$v eq $_} @$i} keys %{$a->{-key}}))
	: ()
	};
    $wk=undef if !%$wk;
 my $o =($a->{-keyord} ||$a->{-orderby} ||$a->{-order})
	|| (!$e && (!@$k) ? '-aall' : '-aeq');
    $o ='-' .$o if substr($o,0,1) ne '-';
 my $ox=@$k ? $o : $e ? $o : $o =~/^-[af]/ ? '-aall' : '-dall';
 my $ws;
	if ($wk) {
		$ws =substr($o, 2);
		$ws =0 ? undef
 ...
 my $wa=$a->{-urole} && !$a->{-uname}
	? mdeRole($s, $m, $a->{-urole})
	: mdeReaders($s, $m);
 my $wr=$a->{-uname} && $a->{-urole} && mdeRole($s, $m, $a->{-urole});
 if ($wr) {
	my $wl	=$wr;
 ...
 my $wf=$a->{-filter};
 my $we=$wc;
 if (defined($wc) && !ref($wc) && $wc) {
	# !!! where string parsing, examples:
	#	$x =~/^[\s\n]*(?<!\\)(?:\\\\)*<[\s\n]*([^\s><\/"]+)[\s\n]*((?:[^<>]|(?<!\\)(?:\\\\)*\\[<>]){0,1})[\s\n]*>[\s\n]*/
	#	$a =~/[\s\n]*([^=]+)="((?:\\"|[^"])*)"[\s\n]*/
	#	$x =~/^[\s\n]*((?:[^<]|(?<!\\)(?:\\\\)*\\[<])*)[\s\n]*((?:<.+){0,1})/
	$we =~s/({\w+}\s*=(?!=))/$1 .'='/ge;
...
sub cgiRun {	# Execute CGI query
 ...
 while(1) {	# Determine / delegate operation object requested
	$s->{-pcmd}->{-form} =$on;
	...
	my $nxt;			# delegate action - substitute object 
 ...
 # !!! recForm called in too diffirent contexts ???
 # !!! cmg transitions ???
 # global       commands
 # -------      --------
 ...
 if	(ref(my $e =$om->{"-$oa"}) eq 'CODE') {
 ...
 elsif	($ot && $oa =~/^rec/) {		# !!! incapsulate action executor ???
	if	($oa =~/^recList/) {
		...
		$s->{-pout} =$s->recRead(-table=>$ot
				...
				, map {($_=>$s->{-pcmd}->{$_})
				} grep {defined($s->{-pcmd}->{$_}) 
					&& $s->{-pcmd}->{$_} ne ''
					}  qw(-key -form));
	}
	else {
		$s->{-pout} =$s->$oa(-data=>$s->cgiDBData($on, $om, $s->{-pdta})
				, -table=>$ot
				, map {($_=>$s->{-pcmd}->{$_})
				} grep {defined($s->{-pcmd}->{$_}) 
					&& $s->{-pcmd}->{$_} ne ''
	...
	$s->{-pcmd}->{-key} =$s->recWKey($ot, $s->{-pout})
	...
	return(&{$s->{-die}}($s->lng(0,'cgiRun') .": Action '$oa\@$og' not found") && undef)
		...
		# Reassign form
 $s->{-pcmd}->{-form} =$on =(isa($s->{-pout}, 'HASH') && $s->{-pout}->{-form}) ||$on;
...
sub cgiParse {	# Parse CGI call parameters
 ...
 foreach my $k (keys %$d) {
	...
	if ($k =~/^_(cmd|cmg|frmCall|frmName\d*|recNew|recRead|recPrint|recEdit|recIns|recUpd|recDel|recForm|recList|recQBF|submit.*|app.*|form|key|qkey|qwhere|qurole|quname|qftext|qversion|qorder|qlist|edit|backc|login|print|xml)(?:\.[xXyY]|__S){0,1}$/i) {
		my ($c, $v) =($1, $d->{$k});
		if ($k =~/^(.+)\.[xXyY]$/) {
			$g->param($1, 1);
			$g->delete($k);
...
sub htmlStart {	# HTTP/HTML/Form headers
 ...
 my $r	=join("\n"
	, $s->{-c}->{-httpheader} =$s->cgi->header(
		...
		? %{$s->{-httpheader}} 
		: ())
	, $s->{-c}->{-htmlstart}  =$s->cgi->start_html(
...
sub htmlHidden {# Common hidden fields
 ...
 join("\n"
	...
	, map { '<input type="hidden" name="_' .$_ .'" value="'
		...
		? qw(qkey qwhere qurole quname qversion qorder)
		: qw(qlist)	)
	) ."\n"
}
...
sub htmlMenu {	# Screen menu bar
 ...
 my $mc =$g ne 'recList'
	...
	: join("; "
	, grep {$_
		} htmlEscape($s, $c->{-qorder} && $c->{-qorder} =~/^-/ ? lng($s, 0, '-qorder') .' ' .substr($c->{-qorder},0) : '')
		, htmlEscape($s, strdata($s, $c->{-qkey}))
		...
		, htmlEscape($s, $c->{-qorder} && $c->{-qorder} !~/^-/ ? lng($s, 0, '-qorder') .' ' .$c->{-qorder} : '')
	);
...
sub cgiForm {	# Print CGI screen form
 ...
 foreach my $f (@{$m->{-field} 
	...
	elsif	(ref($f) eq 'CODE')	{$s->output(&$f($s)); next}
	elsif	(!$f)			{next}
	else				{}
 ...
 if ($qm) {	# Query condition fields
	...
	$s->output($th, htmlEscape($s, lng($s,0,'-qorder')), '</th>', $td
		, htmlField($s, '_qorder', lng($s,1,'-qorder'), {$de eq 'dbm' ?(-labels=>$qo) :(-asize=>50)}, $s->{-pcmd}->{-qorder}||'')
		, "</td></tr>\n");
	$s->output($th, '</th>', $td
 ...
 if ($edt) {				# Edit widget
	...
	.join('; ', 
		map {	my $f =$s->urlEscape($_);
			...
			, $s->cgi->checkbox(-name=>$fnl, -value=>$_, -label=>$_))
		} $s->rfdGlobn($c, $d))
 }
 else	{				# View widget
	$r .=' '
...
sub cgiDDLB {	# CGI Drop-down list box
 ...
 elsif	(ref($d) eq 'HASH') {	# && 0 !!! test 'cgiList' with hash ???
	$s->output($sl, "\n");
...
sub cgiQuery {	# Query records
 ...
 elsif	(ref($a{-where}))	{push @{$a{-where}}, $c->{-qwhere}}
...
sub cgiList {	# List queried records
 ...
 my $limit =$c->{-limit}  || ($m->{-query} && $m->{-query}->{-limit}) ||$m->{-limit} ||512;

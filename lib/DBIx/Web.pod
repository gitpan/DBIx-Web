...
C<-fld>, C<-expr>, C<-lbl>, C<-lblhtml>, C<-form>, C<-cmt>;
C<-flg>, C<-chk>;
C<-hide>, C<-hidel>, C<-edit>, C<-null>;
C<-inp>, C<-inphtml>, C<-ddlb>, C<-ddlbmult>, C<-ddlbtgt>;
C<-colspan>, C<-fhclass>, C<-fhstyle>, C<-fhprop>, C<-fdstyle>, C<-fdclass>, C<-fdprop>;
...
C<-recTrim0A>, C<-recChg0A>, C<-recChg0R>, C<-recChg1A>;
...
C<-recX#A> (C<-recTrim0A>, C<-recChg0A>, C<-recChg0R>, C<-recChg1A>);
...
See also C<-colspan> and C<-rowspan>.
...
Record changing triggers before ('0A') and after ('1A') record data actions.
Called by C<User Interface>.

C<-recChg0A> called after C<-recTrim0A>, C<recNew>, C<recRead> (in edit mode),
and before any of
C<recForm>, C<recIns>, C<recUpd>, C<recDel>.

C<-recChg1A> called after C<recIns>, C<recUpd>.
...
C<recIns> and C<recUpd> trigger before each row.
Executed before C<-recIns0R> and C<-recUpd0R>.
May be used to provide absolutely computed values.
See also C<-recChg0A>.
...
before C<-recChg0R>, C<-recRead0R>, C<-recForm0C>.
...
C<-recNew0R> trigger to add some field values from sample record.
...
=item -ugadd
	...
	=> not exists || [group names] || {group name => alias,...} || sub{}() -> ... 
...
C<-recForm0R>, C<-recChg0R>, C<-recIns0R>, C<-recInsID> triggers executed
...
C<-recNew1C> and C<-recForm1C> triggers are executed at last.

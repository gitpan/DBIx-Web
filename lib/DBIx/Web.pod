...
Should convert C<-wikn>/C<-wikq> parameters to C<-table>/C<-qkey>, may use database.
Else C<-wikn> will be porcessed.
C<-wikq> request parameter may be any value to be processed by C<-wikq> sub{}.
...
-wikq   => wiki query specificator to find record with,
...
New #ftext() and #urole() substitutions for SQL WHERE.

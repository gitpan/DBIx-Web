...
C<-frmLso>, C<-query>, C<-qfilter>, C<-qhref>, C<-qhrcol>, C<-qfetch>, C<-limit>; C<-cgvXXX>)
...
C<-fld>, C<-expr>, C<-lbl>, C<-lblhtml>, C<-form>, C<-cmt>, C<-flg>;
...
C<-recTrim0A>, C<-recChg0A>, C<-recChg1A>;
C<-recForm0C>, C<-recForm1C>;
C<-recNew0C>, C<-recNew1C>;
C<-recIns0R>, C<-recInsID>, C<-recIns1R>;
...
C<-recX#A> (C<-recTrim0A>, C<-recChg0A>, C<-recChg1A>);
...
C<cgiRun> (C<-cgcXXX>, C<-subst>, C<-qlist> vs C<-recList>, C<-recXXX>, C<-cgvXXX>);

C<cgiForm> (C<-field>, C<-query> C<-data>, C<-table>, C<-field>);

C<cgiList> (C<-query> C<-data>, C<-field>, C<-table>, C<-field>)
...
(C<Field Description>, C<Form Description>, C<Table Description>)
...
Additional C<-qwhere> query clause is used as C<recQBF> default,
because all C<-query> clauses may be overwritten via C<recQBF> 
except C<-where> and C<-key> catenation.

Additional C<-qfrmLso> and C<-qurole> options may be used as default C<-frmLso> selection(s).
...
C<-recChg0A> called after C<-recTrim0A>, C<recNew>, C<recRead>,
and before 
C<recForm>, C<recIns>, C<recUpd>, C<recDel>.
C<-recChg1A> called after 
C<recNew>, C<recForm>, C<recIns>, C<recUpd>, C<recRead>.
May be used to provide interactivelly computed values.
...
(C<Database Description>, C<Table Description>, C<Form Description>; all executed)
C<recForm> triggers before ('0C') and after command ('1C').
C<-recForm0C> is executed starting any record data manipulation,
after C<-recNew0C>, C<-recIns0C>, C<-recUpd0C>, C<-recDel0C>;
may be used for some kind of computed values.
...
(C<Database Description>, C<Table Description>, C<Form Description>; all executed)
C<recIns> triggers before before command ('0C'), before row ('0R'),
after row ('1R'), after command ('1C').
Result values are not available in '0R' triggers.
See also C<-recInsID>.
...
Newly created record's hash ref is available in '0C' trigger, which
...
(C<Database Description>, C<Table Description>, C<Form Description>; all executed)
C<recRead> triggers before ('0')  and after ('1') row read.
Result values are not available in before triggers.
...
and C<-display> to use instead of C<-flg> vaules of C<-field>.
...
See C<Table/Form Description> and C<cgiList> for query and list specifications.
...
C<-recForm0C> and C<-recForm1C> triggers executed.
...
C<-recForm0C>, C<-recIns0R>, C<-recInsID> executed;
record inserted into database;
<-recIns1R>, C<-recForm1C> executed.
...
but not used instead of C<-recNew0C>.
Given field values used to initiate new record's hash ref;
C<-rvcInsBy> C<-rvcUpdBy>, C<-rvcInsWhen>, C<-rvcUpdWhen> filled;
C<-recNew0C> and C<-recForm0C> executed;
C<-recNew1C> and C<-recForm1C> executed.
...
Executes C<-recRead0C>, C<-recRead1C>, C<-recForm1C>.
...
See also C<-recUpd0C>, C<-recForm0C>, C<-recUpd0R>, C<-recUpd1C>, C<-recForm1C>.
...
C<-AuthGroupFile> should not be set to C<pthForm>('var','uagroup'),

...
C<ugrexp>, C<ugmember>, C<uadmin>, C<uadmrdr>;
...
C<-rac>; 
C<-racAdmin>, C<-racAdmRdr>, C<-racReader>, C<-racWriter>; 
...
'a'll operation below, except 'k'ey, 'w'key, quoting, default
...
C<-racAdmin> can read only permitted records, but update all can read.

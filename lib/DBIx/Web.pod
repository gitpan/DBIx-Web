...
C<-dbi>, C<-dbiarg>, C<-dbiph>; C<-cgi>; C<-output>;
...
C<-frmOpt>, C<-query>, C<-qfilter>, C<-qhref>, C<-qhrcol>, C<-qfetch>, C<-limit>; C<-cgvXXX>)
...
=item -frmOpt
	...
	=> not exists || boolean || [options list] || sub{}
	sub{}(self, name, meta) -> [list]
	sub{}(self, name, meta, option) -> proceed
...
Additional C<-qfrmOpt> and C<-qurole> options may be used as default C<-frmOpt> selection.

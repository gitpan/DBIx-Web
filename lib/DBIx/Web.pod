...
C<-userln>, C<-usernt>;
C<-udflt>, C<-unflt>, C<-ugflt>, C<-ugadd>;

C<user>, C<userln>, C<unames>, C<ugroups>, C<ugnames>, C<udisp>, C<uglist>;
C<ugrexp>, C<ugmember>, C<uguest>, C<uadmrdr>, C<uadmwtr>, C<uadmin>;
C<w32IISdpsn>, C<w32adhi>, C<w32agf>
...
C<-key>, C<-keycmp>, C<-wkey>, C<-urm>, C<-null>;
C<-ixcnd>, C<-ixrec>;
C<-optrec>;
C<-dbd>;
C<Triggers>;
C<User Interface> (C<-cgcXXX>; C<-subst>, C<-recXXX>;
...
C<-racActor>, C<-racPrincipal>, C<racUser>;
C<-editable>
...
C<-rvcActPtr>, C<-rvcChgState>, C<-rvcCkoState>, C<-rvcDelState>.
...
C<-recTrim0A>;
C<-recForm0A>, C<-recForm0C>, C<-recForm0R>, C<-recForm1C>, C<-recForm1A>;
C<-recEdt0A>, C<-recEdt0R>, C<-recChg0R>, C<-recEdt1A>;
...
C<-logo>, C<-icons>, C<-menuchs>, C<-menuchs1>, C<-frmLso>, C<-frmLsc>;
C<-login>;
C<-cgvXXX>;

C<cgiRun>, C<cgiParse>, C<cgiQuery>; 
C<urlAuth>, C<urlOpt>;
C<psParse>, C<psEval>; 
C<htmlStart>, C<htmlEnd>, C<htmlHidden>, C<htmlMenu>, 
C<cgiForm> (and also C<htmlField>, C<cgiDDLB>, C<tfvVersions>, C<tfdRFD>, C<tfvReferences>),
C<cgiList>, 
C<cgiFooter>.
...
(C<SubObjects>, C<Record level Access Control>)
...
(C<Field Description>)
Form data ('td') HTML tag class - string or sub{$_}($self, command, record)


=item -fdstyle

(C<Field Description>)
Form data ('td') HTML tag style - string or sub{$_}($self, command, record)
...
(C<Field Description>)
Form label ('th') HTML tag class - string or sub{$_}($self, command, record)


=item -fhstyle

(C<Field Description>)
Form label ('th') HTML tag style - string or sub{$_}($self, command, record)
...
Another type entries are tabular form layout hints:
...
=item -form
	...
	=> not exists || form name to display records || true
...
C<-frmLso0A> trigger may be used to change C<-pcmd> 
emulating user defined query parameters.
C<-frmLso0C> trigger may be used to modify immediate C<cgiSel> arguments.

C<-frmLso1C> and C<-frmLso2C> triggers may be used to modify 
C<htmlMenu> comment and C<cgiList> first column header.
C<urlOpt> method may be useful.
...
=item -hide
	...
	=> not exists || sub{$_}({self},{field},edit,query,value) || boolean
...
-loop   => boolean  or  -ddlbloop => boolean	# C<recForm> on list box choice change
...
=item -inphtml
	...
	=> not exists || html text || sub{$_}({self},{field},edit,query,value) -> html
...
(C<Field Description>)
List data ('<td>') HTML tag class - string or sub{$_}($self, cursor)


=item -ldstyle

(C<Field Description>)
List data ('<td>') HTML tag style - string or sub{$_}($self, cursor)
...
=item -lsthtml
	...
	=> not exists || sub{$_}($self, cursor, [record]) -> HTML # local $_ =value
...
'var' - variables file 'var.pl' (see C<varLoad>, C<varStore>), 'uagroup' and 'ualist' files ($^O eq 'MSWin32', C<w32agf>);
...
=item -recDel0R
	...
	=> not exists || sub{}($self, {command}, ?, prev {field => value})
...
Executed after C<-recForm0R> and before C<-recChg0R>.
...
Called by C<User Interface>,
C<-recForm0A> after C<-recTrim0A> but before C<-recChg0A>,
C<-recForm1A> after C<-recChg1A>.

May be used to provide computed for display values.
See also C<-recChg0A>, C<-recChg0R>, C<-recChg1A>; 
C<-recForm0C>, C<-recForm0R>, C<-recForm1C>.
...
Considered in C<w32agf>.
See also C<-unflt>, C<-ugflt>.


=item -ugadd

	=> not exists || [group names] || {group name => alias,...} || sub{$_}() -> ... 
...
but C<recDel> triggers (C<-recForm0C>, C<-recDel0C>, C<-recDel0R>, C<-recDel1C>) will be executed anyway.
...
C<-recForm0C>, C<-recForm0R>, C<-recForm1C> triggers executed.
...
C<-recForm0R>, C<-recEdt0R>, <-recChg0R>, C<-recIns0R>, C<-recInsID> triggers executed
and record is inserted into database.
...
=item recLast (table name||command, data {field=>value,...}, lookup [field name,...],... target [field name,...] || sub{}(self, data) || undef) -> record {field=>value,...}
...
Useful for fetching default field values.
...
C<-rvcInsBy> C<-rvcUpdBy>, C<-rvcInsWhen>, C<-rvcUpdWhen> fields are filled.
Prototype's field values may be obtained via C<recRead>(-table=>name, -key=>{prototype C<recKey>}),
but are not used instead of C<-recNew0R>.
C<-recForm0R>, C<-recEdt0R>, C<-recNew1C>, C<-recForm1C> triggers are executed at last.
...
Executes C<-recForm0C>, C<-recRead0C>, C<-recForm0R>, C<-recRead1C>, C<-recForm1C> triggers.
...
=item recUpd (? -table => name, ? -form => name, ? (-data => {fieldname => value,..}) || (fieldname => value,..), ? -key => {original C<recKey>}, ? -where => 'condition', ? -optrec => false ||true) -> successfully updated {field => value,...}
...
See also C<-recForm0C>, C<-recUpd0C>, C<-recForm0R>, C<-recChg0R>, 
C<-recUpd0R>, C<-recUpd1C>, C<-recForm1C> triggers.
...
=item tfvReferences (?[fields definitions] ||undef, -option=>value,...) -> sub{records references embedded view template}
...
=item tfvVersions (?[fields definitions] ||undef, -option=>value,...) -> sub{records versions embedded view template}
...
=item tvmHistory (?[fields definitions] ||undef, -option=>value,...) -> (database history materialized view template)
...
=item tvmReferences (?[fields definitions] ||undef, -option=>value,...) -> (records references materialized view template)
...
=item tvmVersions (?[fields definitions] ||undef, -option=>value,...) -> (records versions materialized view template)
...
=item uglist (? '-ug<>', ? filter username || groupname || sub{}(self,name), ? container [] || {}) -> [user and group names]
...
Uses C<-AuthUserFile> and C<-AuthGroupFile>, or C<w32agf>.
...
Using C<w32agf>(), C<-AuthGroupFile> and C<-AuthUserFile> should not be set.
This slots should be set only to externally controlled files.
I.e. to files produced via "$0 -call w32agf 0 q" C<Command Line>.
...
Previous values are available only in 'by-row' triggers for existed records.
Special additional command attributes may be: 

-cmd	=> command or method name,

-cmdf	=> form metadata, 

-cmdt	=> table metadata;

and from C<User Interface>:
...
C<-end0>, C<-end1>, C<-endh> triggers created, C<-endh> used by C<w32agf>.

...
#,-AuthGroupFile=>''		# apache groups file
 ,-usernt	=>1		# windows NT style for user names
   ...
   ,'hdesk'=>{		### hdesk table
	...
	,-field		=>[
		...
		,{-fld=>'votimej'
			...
			,-lbl=>'Exec+below', -cmt=>'Subrecords update ordering of records'
			,-lbl_ru=>'Вып+под', -cmt_ru=>'Упорядочение записей по изменению подзаписей'
			,-ldstyle=>$w->{-a_cmdbh_fsvrlds}
			,-ldprop=>'nowrap=true'
			,-lsthtml=>sub{/(?::00|\s00:00:00|:\d\d)$/ ? $` : $_}
			 }
